...
Four small packages that exist so the rest of the gateway does not have to repeat itself. Nothing here knows what a nonce or an escrow is.
...
| `safemath` | saturating `int64` arithmetic, so a counter stops at the bound instead of wrapping past it |
