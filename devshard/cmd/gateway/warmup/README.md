...
- **The probe's timeout kind is read from the receipt, the way the engine reads it.** A host that receipts and then hangs outlives `probeTimeout`, so the send errors and its reply never arrives; the receipt is therefore taken as it arrives. Hardcoding the kind to `refused` filed the expensive failure as the cheap one.
