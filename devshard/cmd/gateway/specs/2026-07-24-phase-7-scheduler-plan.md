...
- `BlockHost(escrowID, participant)`: add to a per-escrow `map[escrowID]set[participant]` (own mutex) that the dispatcher's `stateBlocked` predicate reads (research §2 row 6, §6-8) — permanent, never cleared.
