...
- **The epoch a swept escrow is stamped with is the one the chain stamped on it at creation**, resolved per escrow and memoised, because an escrow's creation epoch never moves (`CreationEpochFunc`, `Recorder.epochOf`). An escrow whose epoch cannot be resolved is not opened at all, so no fact is filed under a guess: a guess is indistinguishable from a fact to everything downstream, and `Book.OpenEscrow` pins the first value it is given for the escrow's life.
  ...
  Stamping the epoch the escrow was *first seen in* is what this used to do, on the assumption that counters start empty on every boot and the two therefore say the same thing. They do not. The counters are restored from `accounting.db`, so "first seen" is a property of that file, and the chain-side half of every escrow — its latest nonce, its lifetime `HostStats`, its chain cost — is re-read whole on every sweep whatever the ledger remembers. The file is dropped and rebuilt whenever `accounting.SchemaVersion` moves, silently and without an error, so a routine deploy re-filed every live escrow's entire history under whatever epoch the gateway restarted in. The same happened through `POST /v1/admin/accounting/reset/{epoch}`, which deletes live escrows and not only retired ones, and through retention pruning an escrow that later came back.
