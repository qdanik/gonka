...
- **`BuildCourier`** — the `transport.ClientConfig` the session's host clients dial with: an anchor scheduler and the peer-tip cache it stamps from. `nil` when height sync is off, so the gateway dials exactly as it did before.
- **`StartCadence`** — starts the session's heartbeat loop, which also starts its seed loop. The session's `Close` stops both. What interval it runs at is the runtime-params feed's answer, not this package's.
...
- **The scheduler and the clients must share one cache.** A scheduler built over a second, empty cache stamps nothing, and the fault shows only as a quiet escrow that never syncs. `BuildCourier` returns both halves together for that reason, and `user/httpsession.go` passes the cache to every client of the session.
...
- **The cadence is on unless the fleet is told it is not.** devshardctl started the loop unconditionally, and a deployment upgrading into this gateway must not go quiet because nobody set a new flag. `GATEWAY_HEIGHT_SYNC_ENABLED=false` is for the fleet whose hosts do not carry height sync, which would otherwise skip a heartbeat every interval and log it.
