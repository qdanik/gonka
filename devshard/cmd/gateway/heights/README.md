...
`GATEWAY_HEIGHT_SYNC_ENABLED` (on), `GATEWAY_HEIGHT_SYNC_REQUIRE_SEED` (on), `GATEWAY_HEIGHT_SYNC_CHAIN_ORACLE` (off), `GATEWAY_HEIGHT_SYNC_ANCHOR_K` (10), `GATEWAY_HEIGHT_SYNC_ANCHOR_SLOTS` (1). The follower also reads the fleet's `NODE_MANAGER_ADDR`. The cadence's own schedule is not a knob here: it comes from governance through [`runtime_params.go`](../runtime_params.go). See [`docs/operations.md`](../docs/operations.md).
