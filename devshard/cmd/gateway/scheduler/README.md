...
- **The balance floor** prices the reserve the way the chain does, `(input_length_bytes + max_tokens) × token_price`, and asks whether the escrow still covers everything in flight plus this arrival. The input side is the body's byte length, not the char/4 estimate the congestion windows use, and the output side is the larger of what this request reserved and the configured cap, because a per-model cap or an admin request can carry a `max_tokens` far above it. Every multiplication is overflow-checked, because a price this escrow cannot afford must not read as an affordable small one.
