...
- **Seven host gates, in this order**: outside the allowlist, proof-of-compute-required, congestion windows full, cut off, ejected, state-diverged, then excluded by this waiter. A participant named by `scheduler_unthrottled_participants` is not asked the first, third and fifth of those; the other four hold for everybody. See routing.md, "Unthrottled participants". The first six need no waiter and are memoised once per drain; only the seventh depends on who is asking, which is why `match`, `servable` and the escrow pick's burn forecast read the same `blocks` definition rather than three ladders. The forecast reads it unmemoised and across escrows, because the memo is keyed by participant alone and one escrow's state block is not another's. A capability refusal is counted, never routed on.
...
- **The score** is the ascending utilisation ratio `(ActiveUsers + expectedBurns) / EscrowWeight`. A non-positive or NaN weight scores unusable rather than best.
- **`expectedBurns` is the distance from the escrow's nonce cursor to a slot that can take this request**, walked over the same `blocks` ladder the drain reads, past the waiters already queued on that escrow. It is added to the in-flight count rather than multiplied into the ratio, because an escrow with nothing in flight would otherwise score zero however far its cursor sits from a usable host. A group no slot of which can take the request is priced at one lap and still picked when it is the only one: the pick predicts, the drain decides, and the drain's sweep refuses without spending a nonce. See [routing.md](../docs/routing.md), "Pricing an escrow by the burns it will cost".
- **A drain that answers `ErrHostsBusy` earns one more escrow.** `Pick` routes a second time, once, excluding the escrow that gave up, and only for an unpinned request; when the second round finds nowhere to go the first escrow's answer stands, because a busy shard must not read as a missing one. See [routing.md](../docs/routing.md), "One re-pick when an escrow gives up".
...
- **The nonce cap** uses `fallbackNonceCeiling` — the fixed ceiling applied while governance `max_nonce` is unread — until governance `max_nonce` has been fetched. A fetched cap is lowered by `nonceInFlightMargin`, or by half the cap where that is smaller. The fetched value is clamped to `MaxUint32`, never allowed to wrap: a cap wrapping to 0 makes `MaxActiveNonce` return `^uint64(0)` and disables the gate.
- **The balance floor asks two questions and prices them differently.** "Can this escrow pay for this request" is priced the way the chain charges it, `(input_length_bytes + max_tokens) × token_price`, and only skips the candidate. "Is this escrow finished" is what reports the escrow for replacement, so it is priced on one capped answer and reads nothing from the arriving request — otherwise one oversized arrival retires every escrow of the model. See [capacity.md](../docs/capacity.md), "The balance floor". Every multiplication is overflow-checked, because a price this escrow cannot afford must not read as an affordable small one.
- **Reserves rank last, and a take is reported once it served.** Regular escrows are ranked first; only when that tier picks nothing, and passed no regular over for busy hosts, an unusable weight or an allowlist that admits none of its group, is the same walk run over the model's reserves. A reserve is reported through `Deps.OnReserveTaken` only once the dispatcher has handed the request a nonce on it (`request_pick.go`, `pickOnce`), so a pick that fails leaves it a reserve. See [routing.md](../docs/routing.md), "A reserve escrow".
- **Exhaustion is reported, not just acted on.** Routing only declines an exhausted escrow; replacing it belongs to the rotation lifecycle, which otherwise never learns and lets the escrow drain silently into `ErrNoEscrowCapacity`. The fallback ceiling is the one reason never reported, so an escrow past it is reported only once the balance floor catches it too. An escrow the balance floor caught while it could still refuse cleanly is reported as the running dry it is, wrapping `ErrInsufficientBalance`, rather than as a model nobody serves.
