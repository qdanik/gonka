...
The long-response exemption is what those numbers interact with. An attempt that produced content, left its nonce open and ran past 280 seconds is judged by neither ladder — no performance sample and no limiter verdict — because at that point the delay is a slow answer rather than an unresponsive host (`engine/outcome.go`, `RaceOutcome.longResponseExempt` and `exemptOnBothLadders`). The loser grace is longer than the exemption, so a loser cancelled after it went quiet has usually run past it.
