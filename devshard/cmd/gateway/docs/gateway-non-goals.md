...
**The non-streaming reduced-max-tokens retry.** Sixty lines that rewrote the caller's body and retried with a smaller `max_tokens`. A race already escalates to another host on `attempt_failed`, and the no-content backstop still applies, so the retry was buying a second chance on the same host for a request another host was already being given. This is a deliberate reduction of the behaviour surface.
