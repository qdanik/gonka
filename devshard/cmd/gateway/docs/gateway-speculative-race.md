...
| `response_timeout_retry` | Non-streaming only, at most once per race. The host receipted and has produced nothing after a wait proportional to the answer it was asked for (`min(non_stream_response_ceiling_ms, max(non_stream_response_floor_ms, output_tokens x per_output_token_response_lag_ms))`), so one more attempt is started with the budget the client asked for. |
...
**The halved retry is the only escalation a buffered answer can have.** A non-streaming request reveals nothing until it is complete, so neither the first-token deadline nor `attempt_failed` can help it: the first has no token to wait for and the second fires only after the attempt is over. The signal available is the answer the request asked for, which is why the deadline grows with the output budget and is floored so a small budget is not retried early. The prompt does not enter it: prefill is what `receipt_timeout` already bounds, and a deadline that grew with input put a 6k-token request's only retry two minutes out, past the point any client is still waiting. The ceiling bounds it from the other side, because a retry started after it can cost a nonce and still not arrive before the client gives up. The retry carries the client's own budget. Halving it was the original theory — half the tokens generate in half the time — but production measured the gap between a stuck host and a fresh one at thirteen times, where halving buys two: the full retry wins the same races and returns the whole answer. It cost a quarter of served requests a silently truncated reply. The three timings live in `config.Engine` as `engine_non_stream_response_floor_ms`, `engine_non_stream_response_ceiling_ms` and `engine_per_output_token_response_lag_ms`, and the one-shot is consumed when the rung fires.
