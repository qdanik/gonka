...
| `response_timeout_reduced_max_tokens` | Non-streaming only, at most once per race. The host receipted and has produced nothing after a wait proportional to the answer it was asked for (`min(non_stream_response_ceiling_ms, max(non_stream_response_floor_ms, output_tokens x per_output_token_response_lag_ms))`), so one more attempt is started with the output-token budget halved. |
