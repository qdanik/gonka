...
- `Engine.Stop` is a real barrier: a race is registered under the engine mutex *before* it starts and released only after its vote goroutine finishes (`engine/engine.go`, `Engine.admit` and `raceRegistration.release`). A barrier over already-finished races would not protect the invariant it exists for.
