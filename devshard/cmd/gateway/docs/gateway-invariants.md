...
**A ninth broke it with a unit mismatch that no type could catch.** A burned nonce is only ever settled by a refusal timeout, and a verifier gates that vote on `nowSeconds - record.StartedAt >= RefusalTimeout` (`host/timeout.go`, `VerifyRefusedTimeout`). Ghost params stamped `StartedAt` with `UnixMilli`, so that difference sat around `-1.8e12` and stayed there: every verifier rejected the vote, at every moment, for every ghost the gateway ever burned. The nonce was committed, could never be timed out, and the host was never charged the miss — a leak that grows with every burn and is invisible until someone counts applied timeouts against burns. Both stamps are `int64` seconds-since-epoch by convention alone, so nothing but the convention distinguishes them; the fix is one call (`registry/session.go`, `nonceStream.ghostParams`) and the guard is a test that measures the difference a verifier measures rather than restating the expression. The test that existed asserted `StartedAt == now.UnixMilli()` — it restated the implementation, so it pinned the defect instead of catching it.

**A tenth tied the executor's receipt to the fate of the reply that was supposed to carry it.** An execution timeout is the only vote that settles a nonce the executor confirmed, and a verifier will only cast it for a record in `StatusStarted` (`host/timeout.go`, `VerifyExecutionTimeout`). Nothing but `MsgConfirmStart` moves a record out of `StatusPending`, and that message is built from the executor receipt inside `processResponse` — which runs when the stream ends. Two consequences followed, and the second is the worse one. A host that holds a stream open outlives the diffs that could have carried the receipt: pending transactions ride the *next* composed diff, so a receipt queued after the escrow's last nonce has nothing left to ride. And a stream that ends without a reply at all — a cancelled client, an aborted upstream — never reaches `processResponse`, so the receipt is discarded with the reply, however much traffic follows. Either way the record stays `Pending`, its execution timeout is refused by every verifier, and the nonce is committed with no path left to settle it.

The receipt does not depend on the reply: it arrives earlier, as its own SSE event, and the transport has it parsed and stored before anything can go wrong downstream. So it is queued there — `Session.SendOnly` wraps the caller's receipt callback and commits `MsgConfirmStart` the moment the event lands (`user/session.go`, `Session.confirmStartOnReceipt`), which required the transport to hand the partially-filled response to that callback (`transport/client.go`). The apply in `processResponse` stays as it is: `addPendingTx` deduplicates by `confirm:<id>`, so the late path is a no-op behind the early one rather than a second source of truth.
