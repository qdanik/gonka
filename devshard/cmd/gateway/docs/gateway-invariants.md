...
**One outcome.** `report()` is the sole caller of `Report`, and it closes the race's `done` channel first (`engine/race.go`). A second call would panic on the double close, which is the structural proof that it happens once. The three possible callers are made mutually exclusive by the race exit value: only `exitComplete` reports on the calling goroutine, while the other two hand a race that still owes work to a single spawned goroutine that awaits and reports (`engine/race.go`).
...
- Escrow id is safe on gauges, where stale series expire, and expensive on counters: escrows rotate every epoch and old ids never return, so a counter keyed by escrow id grows for months. Three counters pay that cost deliberately — the scheduler's ghost-burn, nonce-hold and burn-budget families — because a spent nonce has to be attributable to the escrow that paid for it. Nothing else does.
