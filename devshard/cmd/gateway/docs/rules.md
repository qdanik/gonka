...
- **No changes to the shared `devshard/` packages, except where money is lost there.** The rule holds for anything the gateway can normalise at its own boundary, and it held for the whole rewrite. It was lifted three times, for defects whose cost is a nonce settled at full reserve and which no boundary can normalise away: the execution-timeout sweep, the confirm stamp read from the record rather than the session's cache, and the sentinel that names an unposted vote (see [`findings.md`](./findings.md)). Each fixes the shared packages for the legacy gateway too, which is the test for whether the exception applies. The standing example of the rule itself: the shared timeout handler returns a non-nil error on its own success path, so a posted vote is recognised by the handler's `Applied` flag rather than by the error (`api/timeouts.go`, `engine/session.go`).
