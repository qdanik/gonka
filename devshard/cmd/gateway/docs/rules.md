...
- **Two per-escrow recovery tools are not served**: `signatures/collect` and `sync-hosts`. The read-only half of the recovery surface **is** served, and resolves through the settlement lookup so a draining escrow still answers — which is the point, since the escrow needing inspection is usually the one in trouble.
