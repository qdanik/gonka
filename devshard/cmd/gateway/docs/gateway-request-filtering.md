...
vLLM also builds the reasoning parser from the same kwargs, so it falls back to identity and the whole answer lands in `content`. The test is an identity test against the literal boolean — Moonshot's hosted-API shape `{"thinking": {"type": "disabled"}}` is a dict, not `false`, and leaves reasoning on. A client that put its own `thinking` in `chat_template_kwargs` normally keeps it — the mirror fills the field and never overwrites it — but not here: below the cutoff the gateway overwrites it to `false`, with the same force it already overwrites a client's `thinking_token_budget`. The two have to move together. The `thinking` rule runs at PreValidation and has already mirrored the client's answer into the kwargs by the time the budget is forced at PostLimits, so a fill-only write would leave the template reasoning with a zero budget to reason in — the empty-content burn this whole section exists to prevent, and the half that survives speculative decoding.
