...
Every restart starts clean: no ejections, no capability counts, every AIMD window at its initial value, every breaker closed, every decayed counter at zero. That is a deliberate divergence from the legacy gateway, argued in [rules.md](./rules.md): minute-scale backoff self-heals faster than replaying stale penalties is worth. The cost is that a genuinely bad host gets one free window after every deploy.
