...
**Additive increase, multiplicative decrease.** A success widens the window by one, up to the maximum — but only when the host is actually being used, so an idle host does not accumulate an imaginary window. The utilisation test is `peakInflight ≥ window/2`, where the peak is the highest concurrent count the host has reached since the last widening; a widening resets it to the live count so the next rung has to be earned again. A 429 or 503 halves the window unconditionally, down to a floor of `host_min_inflight` (`limits/participant.go`, `ParticipantLimiter.OnResult`). An upstream 5xx halves it the same way under its own verdict, `UpstreamFault`: the host answered, so the window contracts, but the count of unanswered faults the breaker opens on is left alone — a throttle clears that count, and a host interleaving 5xx answers with connection resets would otherwise never trip the breaker.

Only host-attributable verdicts move the window. A model outcome — a burn-empty, an error stream, a capability refusal — returns before the lock is even taken and does not create state (`limits/participant.go`, the `Verdict` constants and the `ModelOutcome` early return in `ParticipantLimiter.OnResult`). An empty stream is not one of them: the host answered with nothing a client can render, so its window halves under `EmptyAnswer`, the breaker's count left alone as for a 5xx, and the race hands the request to another host. An empty stream that also left the nonce open is `EmptyAnswerLeftOpen`: the host took the work and produced nothing, so the window halves and the fault counts towards the breaker as well.

**Lateness narrows the window too.** A host that misses the receipt or first-token deadline of an attempt still running is halved at that moment under its own verdict, `MissedDeadline`, which leaves the breaker's count alone because the attempt is judged again when it ends. The answer it eventually sends is a `LateSuccess`: it clears the count and lifts a half-open probe like a success, but does not widen the window (`limits/participant.go`, `ParticipantLimiter.OnResult`). Without it a host that answers after minutes counts as a success and its window grows while clients wait on it; with it the window a host keeps is how much it takes on while still starting its answers in time. When and why the race judges a deadline is in [race.md](./race.md), "Deadlines".

**No halving goes below `host_min_inflight`, four by default.** An attempt on a host that answers late or empty costs a nonce for the attempt that replaces it, and a late one costs the client the wait before it; a nonce burned because the host's window is full costs only the nonce, and the request moves on at once. Four still leaves a host that recovers enough work to earn its window back, one slot per timely answer. A half-open cutoff admits its single probe whatever the floor.

**The breaker** trips after a configured number of consecutive transport faults, or immediately if a half-open probe fails — a probe gets exactly one try. Backoff is `base × 1.6^count` clamped to the maximum, with up to 20% jitter added after the clamp so saturated hosts do not all reopen on the same tick; the count stops incrementing once saturated so the exponent cannot overflow (`limits/participant.go`, the `TransportFault` branch of `ParticipantLimiter.OnResult`). The jitter is gRPC's connection-backoff constant, and it exists so that breakers that opened together do not retry in lockstep.
...
The three admission defaults are sized from measured load. **Hold grace is 2 s**: at 1618 nonces burned for nobody against 1607 client requests in 24 hours — one wasted nonce per request, 35% of them arriving at a participant whose window was full and 21% held for a host no queued request would accept — a 200 ms grace is shorter than the gap between arrivals, so the nonce burns before its request arrives. **The initial window is 64 per participant**: no host sustains more than 59 concurrent streams over three days, while a smaller window leaves three participants carrying sixteen slots capped at 12 fleet-wide concurrent until AIMD grows it, which under 200-second requests takes hours. **Queue depth is four**: a slot is held 105 s at the median, so a five-minute budget drains about three deep.
...
Every restart starts clean: no ejections, no capability counts, every AIMD window at its initial value, every breaker closed, every decayed counter at zero. The divergence from the legacy gateway is argued in [rules.md](./rules.md): minute-scale backoff self-heals faster than replaying stale penalties is worth. The cost is that a genuinely bad host gets one free window after every deploy.
...
| `host_min_inflight` / `host_initial_inflight` / `host_max_inflight` | 4 / 64 / 256 | How many requests may be in flight to one host: the least a halving leaves, the start and the most. The window opens near a host's known capacity and AIMD is left to back off from it, rather than discovering it upward from a cold start. |
