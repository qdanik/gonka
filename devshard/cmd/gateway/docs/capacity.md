...
| `429` or `503` | soft | soft | counts when the host carried nothing else of ours for this model as the refusal landed, untouched on the race's report |
| a burn-empty the host held past the refusal timeout | soft | soft | counts when the host carried nothing else of ours for this model as the refusal landed, untouched on the race's report |
| an attempt the twenty-minute backstop cut | soft | soft | counts when the host carried nothing else of ours for this model as the refusal landed, untouched on the race's report |
...
**The cut-off** trips after `host_cutoff_after_failures` consecutive faults of the kinds that count towards it — a transport fault, an empty answer that left its nonce open, and an overload from a host that carried nothing else of ours for this model when it refused — or immediately when a half-open probe fails, because a probe gets exactly one try. An overload is split on what the host was carrying at the moment it refused, which the race reads live rather than at its report, because by the time a race reports the host's other requests may have ended or new ones landed: the coordinator calls `CountRefusalIfIdle` as it handles the attempt's `AttemptDone` (`engine/report.go`, `raceCoordinator.countRefusal`). One that refused while this gateway had nothing else on it for this model is refusing work rather than full of ours, so it counts, and every host answering `503` stops costing a nonce per request once its cut-off opens; one still carrying other requests of ours is busy, so its refusal leaves the count where it was rather than clearing a streak of faults it did nothing to end. The race's report then narrows the windows and leaves the count alone, so no refusal is counted twice (`limits/reaction.go`, `ParticipantLimiter.CountRefusalIfIdle` and `ParticipantLimiter.OnResult`). Backoff is `base × 1.6^count` clamped to the maximum, with up to 20% jitter added after the clamp so hosts saturated together do not all reopen on the same tick; the count stops rising once saturated so the exponent cannot overflow the duration (`limits/reaction.go`, `ParticipantLimiter.applyBreakerLocked`). The jitter is gRPC's connection-backoff constant.
...
**The growth gate reads a peak, and that is what makes call order irrelevant.** `Acquire` records each window's in-flight high-water mark at the instant it takes the tokens, where nothing can undo it, and `OnResult` compares that peak — not the live count — against half the window (`limits/participant.go`, `window.take` and `ParticipantLimiter.growLocked`). This matters because the engine releases an attempt's lease before it announces the attempt is done and reports its verdict afterwards: a gate reading the live count would see the tokens already given back and refuse to widen a window that had been genuinely saturated. Reading the peak decides identically whichever of release and result runs first, so for growth the order of the three calls is an implementation detail and **not** a contract. The live refusal count is the one reading that does depend on order: `runAttempt` gives the lease back before it emits `AttemptDone`, and the coordinator counts a refusal as it handles that event, so the in-flight count `CountRefusalIfIdle` reads is the host's other work for this model at the moment of the refusal (`engine/attempt.go`, `runAttempt`).
...
*Is this escrow finished?* is a fact about the escrow, and it is the question whose answer takes the escrow off the candidate list — the rotation lifecycle mints a replacement and either deactivates the old one or, while it can still recover its own held money, leaves it published and on hold instead (`escrow/depletion.go`, `OnBalanceExhausted`; `escrow/hold.go`, `holdOrPark`; see [routing.md](./routing.md), "An escrow on hold"). So it is priced on one capped answer and reads nothing from the arriving request, whichever of the two the tick ends up choosing. Pricing it on the request instead lets a single arrival take an escrow that is perfectly solvent out of routing, and because the pick walks every candidate against the same price, one oversized arrival takes *every escrow of the model* out of routing in a single pick. Both terms of a request price are unbounded in the way that matters: a body reaches the ingest cap, and the chain counts that body's **bytes**, while an admin request's `max_tokens` reaches ten million.
...
An escrow on hold is a different reason for a low count from the one this section warns about: it is still published and its `devshard_runtime_active` still counts whatever it has left in flight, but nothing new is arriving because it dropped off the candidate list, not because settlement is behind. `devshard_gateway_escrow_on_hold{devshard_id,model}` is the gauge that tells the two apart (`metrics/registry_collector.go`); see [routing.md](./routing.md), "An escrow on hold".
