...
**Additive increase, multiplicative decrease.** A success widens the window by one, up to the maximum — but only when the host is actually being used, so an idle host does not accumulate an imaginary window. The utilisation test is `peakInflight ≥ window/2`, where the peak is the highest concurrent count the host has reached since the last widening; a widening resets it to the live count so the next rung has to be earned again. A 429 or 503 halves the window, with a floor of one, unconditionally (`limits/participant.go`, `ParticipantLimiter.OnResult`). An upstream 5xx halves it the same way under its own verdict, `UpstreamFault`: the host answered, so the window contracts, but the count of unanswered faults the breaker opens on is left alone — a throttle clears that count, and a host interleaving 5xx answers with connection resets would otherwise never trip the breaker.

Only host-attributable verdicts move the window. A model outcome — an empty stream that closed its nonce, a burn-empty, an error stream, a capability refusal — returns before the lock is even taken and does not create state (`limits/participant.go`, the `Verdict` constants and the `ModelOutcome` early return in `ParticipantLimiter.OnResult`). Such an empty stream is what the model produced, not what the host failed to carry, so narrowing the host's window for it would penalise the wrong party. An empty stream that left the nonce open is the other case: the host took the work and produced nothing, and it reaches the breaker as a transport fault.
