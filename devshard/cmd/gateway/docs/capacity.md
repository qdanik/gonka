...
- **The output side is the larger of the request's own reservation and the configured cap.** `MaxTokens` is what this request reserved, which is what the chain is handed — and it can sit far above `max_tokens_cap`, because a per-model cap replaces the global one and an admin request skips the cap entirely (`filters/rules_tokens.go`, `capOutputTokens`). Pricing at the cap alone under-budgets precisely those requests, by as much as the difference between 4 096 and the ten-million ceiling. Taking the larger of the two keeps the floor conservative for everything else.
