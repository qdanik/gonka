...
Neither number is a ceiling: a host that keeps answering grows past both, and one that struggles is narrowed back to what its weight bought and no further. Because the fleet's own door takes the hosts' room as its floor, that growth is admission the gateway can actually use rather than capacity that queues behind a cap priced for a cold fleet. A host the chain has no weight for keeps the flat `host_input_window`/`host_output_window` request counts, which is the behaviour before weights were priced (`limits/participant.go`, `windowsForLocked`). Weights arrive on the phase snapshot the capacity model already watches, taking the lower of the chain's current and full views so a current weight above the full one cannot buy a wider window (`main.go`, `participantWeightsOf`).

It bites where a single request is priced above the whole window — a prompt longer than the context length its model is priced at, or a window a run of bad answers has left at its floor.
...
**Two prices, because the floor is asked two different questions** (`scheduler/scheduler.go`, `requestReserve` and `retirementReserve`).
