...
For a row on hold, `settleHold` asks three questions in order (`hold.go`):
...
2. **Otherwise, is it ready to resume?** `Verdict` (`escrowHolds.Verdict`, defined in `routing.go` and wired in `main.go`) asks the scheduler's own pricing rather than a copy of it: `HoldNonceSpent` parks the row, because a nonce-exhausted escrow can never serve again; `HoldResume` calls `ResumeFromHold` then `registry.Resume`.
3. **Otherwise, has everything it waits for had its chance to come back?** `ReservationsReturnBy` (`registry/views.go`) gives the moment the last `Pending` reservation could have been returned by the race's refusal vote; once that moment and one more tick have passed, `expireHold` parks the row and `EscrowHoldExpired` narrates the balance and whatever is still reserved: money that will not come back before settlement. A `Started` or `Challenged` record has no such moment — the sweep or the dispute returns it only while the escrow is published — so its hold waits for them or for the epoch. Anything else leaves the row on hold for another tick.
