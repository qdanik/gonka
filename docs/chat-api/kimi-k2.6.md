...
| `thinking_token_budget` | int | Resolution order: (1) if `max_tokens < 256`, force `0` **and** set `chat_template_kwargs.thinking = false`, so thinking is off in the template as well as in the sampler; (2) else if absent, default to `max_tokens / 2`; (3) cap at 96 000 (Moonshot HLE/AIME budget); (4) clamp to `max_tokens − 64` so visible content always has headroom after `</think>`. The budget alone is a vLLM logits processor and is discarded wholesale when the serving node runs speculative decoding [[vLLM-29]](references.md#vllm) [[vLLM-30]](references.md#vllm); the template branch does not depend on one [[Moonshot-6]](references.md#moonshot). Stays top-level — it is a declared vLLM request field [[vLLM-32]](references.md#vllm). See [troubleshooting](troubleshooting.md#kimi-empty-content-think-burn). | [[Moonshot-3]](references.md#moonshot), [[OpenAI-4]](references.md#openai) |
