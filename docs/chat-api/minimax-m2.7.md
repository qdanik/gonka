...
| `enable_thinking` (top-level) | translated to `chat_template_kwargs.enable_thinking` on Qwen | **silent-strip on this route** — MiniMax does not honor; thinking is structural to the chat template, not configurable per request ([[vLLM-25]](references.md#vllm)). A `reasoning_effort` the caller sends still makes vLLM derive the same kwarg downstream; it lands unused for the same reason. | [why](troubleshooting.md#strip-enable_thinking-minimax) |
...
|-------|------|----------|--------|
| `extra_body.reasoning_split` | bool | **Defaulted to `true` on this route** when the caller does not send it; a caller-supplied value is kept. If `true`, vLLM/MiniMax emit reasoning as a separate `reasoning_details[]` array on the response instead of inline `<think>...</think>` blocks in `content`. M2.x thinks unconditionally, so the choice is only where the reasoning lands. Gateway unwraps `extra_body` per universal contract; the lifted key passes through to vLLM. Document client responsibility: round-trip `reasoning_details` (or `<think>` blocks) verbatim into history. | [[MiniMax-5]](references.md#minimax) |
