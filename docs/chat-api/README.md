...
| `logprobs` | bool | — | must be a boolean (else reject — [why](troubleshooting.md#reject-malformed-param-types)); pass-through. Returned only when `top_logprobs` also asks | [[OpenAI-1]](references.md#openai) |
| `top_logprobs` | int | — | must be a non-negative integer (else reject — [why](troubleshooting.md#reject-malformed-param-types)); a wider ask is capped at `5`; `0` means no logprobs are returned. A narrower ask is accepted but not honored: the host executes at `5` and the response carries all five | [[OpenAI-1]](references.md#openai) |
