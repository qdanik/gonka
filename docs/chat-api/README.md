...
| `logprobs` | bool | — | must be a boolean (else reject — [why](troubleshooting.md#reject-malformed-param-types)); pass-through. `true` returns each token's own logprob | [[OpenAI-1]](references.md#openai) |
| `top_logprobs` | int | — | must be a non-negative integer (else reject — [why](troubleshooting.md#reject-malformed-param-types)); a wider ask is capped at `5`; `0` or absent returns `top_logprobs` as a present-but-empty array, OpenAI's shape for logprobs without alternatives. A narrower ask is accepted but not honored: the host executes at `5` and the response carries all five | [[OpenAI-1]](references.md#openai) |
...
| `min_tokens` | int | 64 | vLLM extension; **floored to 64** — set to 64 when absent or below 64, kept when ≥64 (clamped ≤`max_tokens`, which is itself raised to ≥64); applied identically on inference and validation replay so both generate a comparable token count | [[vLLM-1]](references.md#vllm) |
