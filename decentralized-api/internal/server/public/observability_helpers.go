...
func startObservabilityInferenceRequestContext(ctx echo.Context) (context.Context, *observability.Operation, *inferenceRequestLogging) {
	...
	ctx.SetRequest(ctx.Request().WithContext(requestContext))
	requestLogging := newInferenceRequestLogging(ctx, requestOp)
	requestLogging.logReceived()
	return requestContext, requestOp, requestLogging
}